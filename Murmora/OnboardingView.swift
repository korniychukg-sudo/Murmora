...
struct OnboardingView: View {
    ...
    var body: some View {
        ZStack {
            AuroraBackground(tint: Murmora.primary, secondary: Murmora.accent(.water), animated: true)
            VStack(spacing: 0) {
                Spacer()
                TabView(selection: $page) {
                    ForEach(0..<pages.count, id: \.self) { i in
                        VStack(spacing: 26) {
                            ZStack {
                                Circle().fill(Murmora.primary.opacity(0.18)).frame(width: 250, height: 250).blur(radius: 20)
                                if let ui = MurmoraArtLoader.image(pages[i].0) {
                                    Image(uiImage: ui).resizable().scaledToFit()
                                        .frame(width: 240, height: 240)
                                        .clipShape(Circle())
                                        .overlay(Circle().strokeBorder(Murmora.stroke, lineWidth: 1))
                                        .glow(Murmora.primary, radius: 24, opacity: 0.4)
                                } else {
                                    Circle().fill(Murmora.heroGradient).frame(width: 240, height: 240)
                                }
                            }
                            VStack(spacing: 12) {
                                Text(pages[i].1).font(.system(size: 26, weight: .heavy, design: .rounded))
                                    .foregroundColor(Murmora.ink).multilineTextAlignment(.center)
                ...
                HStack(spacing: 8) {
                    ForEach(0..<pages.count, id: \.self) { i in
                        Capsule().fill(i == page ? Murmora.primary : Murmora.faint.opacity(0.4))
                ...
                Button {
                    ...
                    Text(page < pages.count - 1 ? "Next" : "Enter Murmora")
                        .font(.system(size: 17, weight: .bold, design: .rounded)).foregroundColor(Murmora.bgDeep)
                        .frame(maxWidth: .infinity).padding(.vertical, 16)
                        .background(Capsule().fill(Murmora.gold)).padding(.horizontal, 30)
                }
