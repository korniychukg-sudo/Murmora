...
struct StudioView: View {
    ...
    var body: some View {
        ZStack {
            AuroraBackground(tint: store.hasMix ? Grove.accent(store.dominantCat()) : Grove.primary,
                             secondary: Grove.accent(.water),
    ...
    private var header: some View {
        HStack(alignment: .center) {
            ...
            Spacer()
            EqualizerBars(active: store.isPlaying, color: Grove.gold, bars: 5, reduceMotion: store.reduceMotion)
                .frame(width: 34)
        }
        .padding(.top, 6)
    }
    ...
    private var stage: some View {
        ZStack {
            ...
            if store.activeSounds.isEmpty {
                VStack(spacing: 10) {
                    GroveIcon(glyph: .waveform, size: 40, color: Grove.primary.opacity(0.8))
                    Text("Your grove is quiet")
                        .font(.system(size: 17, weight: .bold, design: .rounded)).foregroundColor(Grove.ink)
                ...
                GeometryReader { geo in
                    ...
                    if store.activeCount > 8 {
                        Text("+\(store.activeCount - 8)")
                            ...
                            .background(Capsule().fill(Grove.bgDeep.opacity(0.7)))
    ...
    private var masterBar: some View {
        HStack(spacing: 14) {
            Button {
                ...
                ZStack {
                    Circle().fill(store.hasMix ? Grove.heroGradient
                                  : LinearGradient(colors: [Grove.faint.opacity(0.4), Grove.faint.opacity(0.2)], startPoint: .top, endPoint: .bottom))
                        .frame(width: 60, height: 60)
                        .glow(Grove.primary, radius: 16, opacity: store.isPlaying ? 0.6 : 0.2)
                    GroveIcon(glyph: store.isPlaying ? .pause : .play, size: 26, color: .white)
            ...
            VStack(alignment: .leading, spacing: 6) {
                HStack {
                    Text("Master").font(.system(size: 12, weight: .bold, design: .rounded)).foregroundColor(Grove.subtle)
                    Spacer()
                    Text("\(Int(store.masterVolume * 100))%").font(.system(size: 12, weight: .bold, design: .rounded)).foregroundColor(Grove.faint)
                }
                VolumeSlider(value: Binding(get: { store.masterVolume },
                                            set: { store.setMaster($0) }), tint: Grove.primary)
            }

            RoundIconButton(glyph: .bookmark, color: Grove.gold, size: 44, iconSize: 18) {
            ...
            RoundIconButton(glyph: .trash, color: Grove.subtle, size: 44, iconSize: 18) {
                Haptic.tap(); withAnimation { store.clearMix() }
            }
            .opacity(store.hasMix ? 1 : 0.4)
        }
        .groveCard(padding: 14, radius: 24)
    }
    ...
    private func categorySection(_ cat: SoundCat) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 8) {
                Circle().fill(Grove.accent(cat)).frame(width: 10, height: 10)
                Text(cat.title).font(.system(size: 17, weight: .heavy, design: .rounded)).foregroundColor(Grove.ink)
                Spacer()
            }
    ...
    private var saveSheet: some View {
        ZStack {
            ...
            VStack(spacing: 18) {
                Capsule().fill(Grove.stroke).frame(width: 40, height: 5).padding(.top, 10)
                Text("Save Scene").font(.system(size: 22, weight: .heavy, design: .rounded)).foregroundColor(Grove.ink)
                ...
                TextField("", text: $saveName)
                    .placeholder(when: saveName.isEmpty) {
                        Text("Name your scene").foregroundColor(Grove.faint)
                    }
                    .font(.system(size: 17, weight: .semibold, design: .rounded))
                    .foregroundColor(Grove.ink)
                    .padding(14)
                    .background(RoundedRectangle(cornerRadius: 14).fill(Grove.card)
                        .overlay(RoundedRectangle(cornerRadius: 14).strokeBorder(Grove.stroke, lineWidth: 1)))
    ...
    var body: some View {
        ...
        VStack(spacing: 10) {
            HStack(spacing: 10) {
                SoundToken(sound: sound, size: 46)
                    .glow(Grove.accent(sound.cat), radius: active ? 10 : 0, opacity: active ? 0.6 : 0)
                VStack(alignment: .leading, spacing: 2) {
                    Text(sound.name)
                        .font(.system(size: 14, weight: .bold, design: .rounded))
                        .foregroundColor(Grove.ink).lineLimit(1).minimumScaleFactor(0.8)
                    if active {
                        EqualizerBars(active: store.isPlaying, color: Grove.accent(sound.cat), bars: 4, reduceMotion: store.reduceMotion)
                            .frame(height: 12, alignment: .leading)
                    } else {
                        Text(sound.blurb)
                            .font(.system(size: 10.5, weight: .medium, design: .rounded))
                            .foregroundColor(Grove.subtle).lineLimit(1)
                    }
                }
                Spacer(minLength: 0)
            }
            if active {
                VolumeSlider(value: Binding(get: { store.mix[sound.id] ?? 0 },
                             ...
                             tint: Grove.accent(sound.cat), height: 8)
            }
        }
        .padding(12)
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(active ? Grove.accent(sound.cat).opacity(0.14) : Grove.card)
                .overlay(RoundedRectangle(cornerRadius: 18)
                    .strokeBorder(active ? Grove.accent(sound.cat).opacity(0.6) : Grove.stroke, lineWidth: active ? 1.5 : 1))
        )
